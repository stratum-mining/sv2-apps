...
impl<T> Sv1JobStore<T> {
    ...
    pub(super) fn activate(&mut self, job_id: String, job: T, clean_jobs: bool) {
        if clean_jobs {
            ...
            if let Some(active_job) = self.active_job.take() {
                if active_job.job_id != job_id {
                    self.past_job_order.push_back(active_job.job_id.clone());
                    self.past_jobs.insert(active_job.job_id, active_job.job);
                }
            }
        ...
        self.active_job = Some(ActiveJob { job_id, job });
    }

    pub(super) fn get(&self, job_id: &str) -> Option<&T> {
        self.active_job
            .as_ref()
            .filter(|active_job| active_job.job_id == job_id)
            .map(|active_job| &active_job.job)
            .or_else(|| self.past_jobs.get(job_id))
    }

    pub(super) fn active(&self) -> Option<&T> {
        self.active_job.as_ref().map(|active_job| &active_job.job)
    }
