...
use std::sync::Arc;
use stratum_core::{
    codec_sv2::{EncodableFrame, SerializedFrame},
...
struct ConnectionIo<F> {
    sender_incoming: Sender<SerializedFrame>,
    receiver_incoming: Receiver<SerializedFrame>,
...
impl Connection {
    ...
    ) -> Result<(Receiver<SerializedFrame>, Sender<F>), Error>
    where
...
mod tests {
    ...
    async fn cancellation_shuts_down_connection() {
        ...
        let responder_task = tokio::spawn(async move {
            ...
            let responder =
                Responder::from_authority_kp(&pub_key, &prv_key, Duration::from_secs(10_000))
                    .unwrap();
            Connection::accept::<MessageFrame<AnyMessageOwned>>(
                stream,
                responder,
        ...
        let (receiver, sender) = Connection::connect::<MessageFrame<AnyMessageOwned>>(
            stream,
            initiator,
            cancellation_token.clone(),
        )
