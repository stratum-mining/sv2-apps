...
# Publishable manifests, deepest paths first, so that a nested crate claims
# its files before the crate that contains it does.
...
for manifest in $MANIFESTS; do
  ...
  status=$(curl -s -o "$RESPONSE" -w '%{http_code}' \
    -A 'sv2-apps-ci (https://github.com/stratum-mining/sv2-apps)' \
