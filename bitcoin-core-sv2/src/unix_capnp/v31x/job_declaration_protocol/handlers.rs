...
impl BitcoinCoreSv2JDP {
    ...
    /// mempool mirror plus that staging area, assembles a test block, sets IPC thread context, and
    /// uses Bitcoin Core's `checkBlock` to validate the block structure. Staged transactions are
    /// only committed to the mempool mirror after `checkBlock` succeeds and only if the chain
    /// tip did not move while `checkBlock` was in flight, so a rejected declaration never grows
    /// shared state and stale transactions never seed a freshly-cleared mirror. Returns success
    /// with current template parameters or an error if validation fails.
